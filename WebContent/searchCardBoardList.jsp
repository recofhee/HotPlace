		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data != null && data.length > 0) {
					$.each(data, function(index, item){
						...
						endPageNo = item.endPageNo;
							...
							result += '<div class="col-md-6 col-lg-4"><div class="card2"><a href="cardBoardServlet?command=board_view&page='+item.pageNo+'&cb_num='+item.cbNum+'"><div class="layer-img">';
				...
				if(prevPageNo != pageNo) {
					pagination += '<a href="?keyword=${keyword}&page='+prevPageNo+'"><i class="fas fa-angle-left"></i></a>';
				}
				for(var p = startPageNo; p <= endPageNo; p++) {
					if(p == pageNo) {
						pagination += '<a href="?keyword=${keyword}&page='+p+'" class="active">'+p+'</a>';
					} else {
						pagination += '<a href="?keyword=${keyword}&page='+p+'">'+p+'</a>';
					}
				}
				if(nextPageNo != pageNo) {
					pagination += '<a href="?keyword=${keyword}&page='+nextPageNo+'"><i class="fas fa-angle-right"></i></a>';
				}
			...
			<div class="cb-list container">
				<div class="searchWrap">
					<form action="searchCardBoardList" autocomplete="off">
						<input id="keyword" name="keyword" type="search" value="${keyword}" placeholder="검색어를 입력해 주세요."/>
						<button id="searchSubmit" type="button" value="">
							<i class="fas fa-search"></i>
						</button>
					</form>
				</div>
				
				<div class="btn-row-fsb">
					<a href="cardBoardList" class="cb-btn">전체 목록</a>
